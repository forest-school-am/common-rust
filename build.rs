...
//! the sha256 of `templates/stand-oidc.js.jinja` is emitted as a crate const,
...
fn main() {
    let template = "templates/stand-oidc.js.jinja";
    ...
    std::fs::write(
        &out,
        format!("pub(crate) const STAND_OIDC_JS_SHA256: [u8; 32] = [{bytes_list}];\n"),
    )
    .unwrap();
}
