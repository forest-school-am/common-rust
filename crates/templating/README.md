...
`common-templating` is a member of the `common-rust` workspace, so the
dependency points at the repository and cargo selects the member by package
name:

```toml
[dependencies]
common-templating = { git = "https://github.com/forest-school-am/common-rust.git", tag = "v0.1.0" }
```
...
**Migration in progress:** the patch still keys on `common-rust-templating.git`,
a URL that predates the merge of these crates into one workspace and was never a
