...
pub(crate) const BOOL_ACCEPTED: &str = "true, false, 1, 0, yes or no";
...
/// `Refusal::variable` is `&'static str` until Refusal moves into this crate;
/// a refusal happens once, on the way to exit(1), so the leak is bounded.
pub(crate) fn variable(name: String) -> &'static str {
...
fn wrong_type(field: &Field, entry: &Entry, accepted: String, error: String) -> Refusal {
    ...
    Refusal::new(variable(source), shown, accepted)
