...
use common_config::{Config, Outcome, Path, Refusal};
...
fn bool_accepts_false_forms() {
    for form in ["false", "0", "no", "FALSE", "No"] {
        assert!(
            !ok(&["--name=x"], &[("APP_VERBOSE", form)]).verbose,
            "{form}"
        );
...
fn bool_invalid_refuses_naming_the_forms() {
    ...
    assert_eq!(refusal.accepted, "true, false, 1, 0, yes or no");
}
...
fn strum_enum_parses_and_refuses_with_declared_accepted() {
    assert_eq!(
        ok(&["--name=x", "--deployment=prod"], &[]).deployment,
        Deployment::Prod
    );
    let refusal = err(&["--name=x", "--deployment=staging"], &[]);
